...
async function updateDataModelInSigma(dataModelId, dataModelSpec) {
    ...
    try {
        const response = await axios.put(requestURL, dataModelSpec, {
            headers: {
        ...
        //console.log('data model updated successfully:', JSON.stringify(response.data, null, 2));
        return response.data.dataModelId;   

    } catch (error) {
        console.error('error updating data model in Sigma:', error.response ? error.response.data : error.message);
        throw error;
    }
